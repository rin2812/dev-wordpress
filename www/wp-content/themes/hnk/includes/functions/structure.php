...
if ( ! function_exists( 'hnk_head_title' ) ) {
	add_filter( 'wp_title', 'hnk_head_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function hnk_head_title( $title, $sep ) {
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'hnk' ), max( $paged, $page ) );
		}
...
if ( ! function_exists( 'hnk_body_classes' ) ) {
	add_filter( 'body_class', 'hnk_body_classes' );

	function hnk_body_classes( $classes ) {
...
if ( ! function_exists( 'hnk_post_classes' ) ) {
	add_filter( 'post_class', 'hnk_post_classes' );

	function hnk_post_classes( $classes ) {
...
if ( ! function_exists( 'hnk_override_page_menu' ) ) {
	add_filter( 'wp_nav_menu_args', 'hnk_override_page_menu' );
	 ...
	 * @return  array
	 */
	function hnk_override_page_menu( $params ) {
		if ( is_page() ) {
...
if ( ! function_exists( 'hnk_extra_menu_items' ) ) {
	add_action( 'theme/after_primary_menu', 'hnk_extra_menu_items' );
	 ...
	 * @return  void
	 */
	function hnk_extra_menu_items( $args ) {
		if ( $args['env'] == 'desktop' ) {
...
if ( ! function_exists( 'hnk_fix_post_title' ) ) {
	add_filter( 'the_title', 'hnk_fix_post_title', 10, 2 );
	 ...
	 * @return  string
	 */
	function hnk_fix_post_title( $title, $id ) {
		$post = get_post( $id );
