...
if ( ! function_exists( 'hnk_post_title' ) ) {
	/**
	 * Display the post title
	 * 
	 * @return  void
	 */
	function hnk_post_title() {
...
if ( ! function_exists( 'hnk_post_meta' ) ) {
	/**
	 * Display the post meta
	 * 
	 * @return  void
	 */
	function hnk_post_meta() {
		...
		?>
			...
			<div class="entry-meta">
				<span><?php esc_html_e( 'By', 'hnk' ) ?></span>
				...
				<span class="entry-categories">
					<?php the_category( _x( ', ', 'Used between list items, there is a space after the comma.', 'hnk' ) ); ?>
				</span>
				...
				<?php if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) : ?>
					...
					<?php else: ?>
						...
						<span class="entry-comments-link">
							<?php comments_popup_link( esc_html__( '0', 'hnk' ), esc_html__( '1', 'hnk' ), esc_html__( '%', 'hnk' ) ); ?>
						</span>

					<?php endif ?>
				<?php endif ?>
				
				<?php edit_post_link( esc_html__( '(Edit)', 'hnk' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
...
if ( ! function_exists( 'hnk_post_content' ) ) {
	/**
	 * Display the post content
	 * 
	 * @return  void
	 */
	function hnk_post_content() {
		if ( ! is_single() && current_post_type_is( 'post' ) ) {
			...
			if ( op_option( 'blog_archive_readmore' ) ) {
				printf( '<div class="readmore"><a href="%s" class="more-link">Đọc tiếp</a></div>',
					get_permalink(),
